...
package body Stars is
   ...
   begin
      ...
      System.The_Stars (System.Last) :=
        (Kind            => Simple_Planet,
         ...
         Y               => 0.0,
         Index_In_System => System.Last,
         Distance        => Random (The_Seed) * (Max_Distance - Min_Distance)
            + Min_Distance,
         Speed           => Sign *
           (Random (The_Seed) * (Max_Speed - Min_Speed) + Min_Speed),
         Angle           => Pi * 2.0 * Random (The_Seed),
         Turns_Around    => Turns_Around.Index_In_System,
         Graphic         =>
           New_Circle
             (0.0, 0.0, 1.0 + Random (The_Seed) * 2.0, White));
   ...
   begin
      ...
      if Handle_Collision then
         for J in 1 .. Stars.Last loop
            if J /= The_Star.Index_In_System
              and then Stars.The_Stars (J) /= No_Star
            then
               declare
                  Dx, Dy : Float;
                  Min_Distance : Float;
               begin
                  Dx := The_Star.X - Stars.The_Stars (J).X;
                  Dy := The_Star.Y - Stars.The_Stars (J).Y;
                  Min_Distance :=
                    Float (Get_Radius (The_Star.Graphic)
                           + Get_Radius (Stars.The_Stars (J).Graphic));
   ...
   begin
      for J in 1 .. Stars.Last loop
         ...
         exception
            when Collision_Exception =>
               Delete (Stars.The_Stars (J).Graphic);
               Stars.The_Stars (J) := No_Star;
         end;
      end loop;
   end Move_All;
