...
generic
   ...
   --  TP is about replacing a non-generic package with a generic one, and use
   --  it in our context - all the implementation is given

   --  X & Y are fields of the type

   type Object_Type (<>) is limited private;

   type Coordinate_Type is digits <>;

   with function Get_X (V : Object_Type) return Coordinate_Type;

   with function Get_Y (V : Object_Type) return Coordinate_Type;

   with function Get_Radius (V : Object_Type) return Coordinate_Type;

   with procedure Set_X (V : in out Object_Type; Value : Coordinate_Type);

   with procedure Set_Y (V : in out Object_Type; Value : Coordinate_Type);
