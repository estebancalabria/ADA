...
package body Invaders.Foes.Kamikazes is
   ...
   begin
      return Result : Kamikaze_Type := (Foe_Type with others => <>) do
   ...
   begin
      ...
      for J in 1 .. 8 loop
         ...
         Set_X
           (V.Sub_Shapes (J),
            Get_X (V.all) + Cos (Angle) * Kamikaze_Radius);
         Set_X2
           (V.Sub_Shapes (J),
            Get_X (V.all) + Cos (Angle) * (Kamikaze_Radius + 2.0));

         Set_Y
           (V.Sub_Shapes (J),
            Get_Y (V.all) + Sin (Angle) * Kamikaze_Radius);
         Set_Y2
           (V.Sub_Shapes (J),
            Get_Y (V.all) + Sin (Angle) * (Kamikaze_Radius + 2.0));
         end;
      end loop;
   end Redraw;
   ...
   is
   begin
      Set_Y (V.all, Get_Y (V.all) - 0.5);

      Set_X (V.all, Get_X (V.all) + V.Speed);

      if Get_X (V.all) not in 100.0 .. 100.0 then
         V.Speed := -V.Speed;
      end if;

      if Get_Y (V.all) < -100.0 then
   ...
   begin
      return (Get_X (V.all), Get_Y (V.all), 5.0);
