...
package Invaders.Foes.Shooters is
   ...
   type Shooter_Type is new Foe_Type with private;
   ...
   procedure Set_Speed
     (V : in out Shooter_Type; Speed : Float);

   function Get_Speed
     (V : Shooter_Type) return Float;

   procedure Set_Color (V : in out Shooter_Type; Color : Color_Type);

private

   type Shooter_Type is new Foe_Type with record
      Speed       : Float := 0.0;
      Main_Shape  : Shape_Id;
