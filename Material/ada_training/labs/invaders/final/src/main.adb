...
begin
   ...
   Register_Listener (Game'Access, Object_Deleted_Listener_Access (Player));
   ...
   while not At_End loop
      ...
      if Last_Foe > 50 and then Random (Seed) > 0.995 then
         if Random (Seed) > 0.9 then
            if Random (Seed) > 0.5 then
               New_Foe := new Smart_Shooter_Type'(Create);
               Smart_Shooter_Type (New_Foe.all).Aim := Player;
            else
            ...
            begin
               Shooter.X := Length (Random (Seed) * 180.0)  - 90.0 * m;
               Shooter.Y := Length (Random (Seed) * 50.0) + 50.0 * m;

               if Random (Seed) > 0.5 then
                  Shooter.Current_Speed := 0.1 * m / s;
               else
                  Shooter.Current_Speed := -0.1 * m / s;
               end if;
            ...
            begin
               ...
               New_Foe.X := Length (Random (Seed) * 180.0) - 90.0 * m;
               New_Foe.Y := 100.0 * m;
