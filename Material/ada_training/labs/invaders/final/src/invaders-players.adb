...
package body Invaders.Players is
   ...
   begin
      ...
      Set_X (V.Shield_Shape, Float (V.X));
      Set_Y (V.Shield_Shape, Float (V.Y));
      Set_Inner_Radius (V.Shield_Shape, Float (V.Shield_Width) / 2.0);
   end Redraw;
   ...
   is
      ...
      Lateral_Movement : constant Length := 1.0 * m;
   ...
   begin
      ...
      if To_Special (Key) = KEY_LEFT then
         ...
         begin
            Ammo_Rec.X := V.X;
            Ammo_Rec.Y := V.Y + 5.0 * m;
            Ammo_Rec.Kills := Enemies;
      ...
      V.Last_Shoot := V.Last_Shoot + 1;
       ...
       if V.Shield_Width < 1.0 then
         ...
         V.Radius := Length (12.0 + Get_Inner_Radius (V.Shield_Shape));
      end if;
   end Iterate;
   ...
   begin
      if Obj.all in Ammo_Type'Class
        and then Ammo_Type'Class (Obj.all).Kills = Enemies
      then
         return;
      end if;
